...
test("advisor transcripts surface as read-only advisor rows", () => {
  const { dir, sessionFile, artifactsDir } = makeSessionFixture();
  try {
    writeFileSync(join(artifactsDir, "__advisor.jsonl"), '{"type":"session","version":3}\n');
    writeFileSync(join(artifactsDir, "__advisor_watchdog.jsonl"), '{"type":"session","version":3}\n');
    writeFileSync(join(artifactsDir, "unrelated.jsonl"), '{"type":"session","version":3}\n');

    const roster = extractSubagentHistory(sessionFile);
    const advisors = roster.filter((entry) => entry.kind === "advisor");
    assert.deepEqual(advisors.map((entry) => entry.id).sort(), ["__advisor", "__advisor_watchdog"]);
    // Advisor rows sort after every task-spawned agent.
    assert.equal(roster.at(-1).kind, "advisor");

    const advisor = advisors.find((entry) => entry.id === "__advisor");
    assert.equal(advisor.agent, "advisor");
    assert.equal(advisor.status, "completed");
    assert.equal(advisor.transcriptAvailable, true);
    assert.equal(advisor.sessionFile, join(artifactsDir, "__advisor.jsonl"));
    // Files that are not advisor transcripts never become rows.
    assert.equal(roster.some((entry) => entry.id === "unrelated"), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
